...
describe Grape::Middleware::Formatter do
  ...
  context 'serialization' do
    ...
    context "when a callback is allowed" do
      before do
        @body = {"smells" => "fishy"}
        subject.options[:callback] = 'my_callback'
      end

      it "wraps the JSON response in the callback if given" do
        status, headers, bodies = *subject.call({'PATH_INFO' => '/somewhere', 'QUERY_STRING' => 'meat=steak&my_callback=salmon'})
        bodies.each { |b| b.should == "salmon(#{MultiJson.encode(@body)});" }
      end

      it "returns standard JSON if the callback isn't given" do
        status, headers, bodies = *subject.call({'PATH_INFO' => '/somewhere'})
        bodies.each { |b| b.should == "#{MultiJson.encode(@body)}" }
      end
    end
